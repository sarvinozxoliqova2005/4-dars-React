      ...
      {filteredStudents.map((el) => (
        <Link to= {`/students/${el.id}`} key={el.id} className='max-w-[400px] w-full rounded-lg border-3 border-[white] shadow p-5 hover:scale-105 duration-500 cursor-pointer group'>
         ...
         <div className="w-full bg-gray-300 rounded-full h-2 mt-2">
          ...
          <div className='flex flex-col gap-1 mt-8 pl-2 text-sm text-gray-600'>
            <a href={`tel:${el.Phone}`} className='hover:text-blue-500 flex items-center gap-2'>
                <img className='w-5' src="https://www.iconpacks.net/icons/1/free-phone-icon-519-thumb.png" alt="" />
                {el.Phone}
            </a>
            <a href={`mailto:${el.Email}`} className='hover:text-blue-500 flex items-center gap-2 mt-2'>
            ...
            <a href={el.Linkedin} className='hover:text-blue-500 flex items-center'>
                <img className='w-10' src="https://img.freepik.com/free-psd/social-media-logo-design_23-2151299455.jpg?semt=ais_se_enriched&w=740&q=80" alt="" />
                LinkedIn
            </a>
          </div>
