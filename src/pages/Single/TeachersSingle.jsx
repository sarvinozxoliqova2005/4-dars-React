...
const TeachersSingle = () => {
  ...
  useEffect(() => {
    async function getTeacher() {
      try {
        const res = await axios.get(`https://6921d8fe512fb4140be18e4b.mockapi.io/Teachers/${id}`);
        setTeacher(res.data);
      } catch (err) {
        console.log(err);
      }finally {
  ...
  return (
    <>
      <div className='flex items-center gap-3 pl-5 pt-5'>
       ...
       <Link to="/teachers" className=" hover:underline cursor-pointer">
        <h1>Back To Teachers</h1>
      </Link>
    </div>
  ...
  <div className="bg-white rounded-xl shadow p-6 w-full md:w-1/3 flex flex-col items-center">
    ...
    />
    <h2 className="font-semibold text-lg">{teacher.FirstName} {teacher.LastName}</h2>
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex items-start gap-3 p-4 bg-gray-50 rounded-xl">
          ...
          <div>
            ...
            <p className="text-gray-600">@{teacher.Telegram}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex items-start gap-3 p-4 bg-gray-50 rounded-xl">
          ...
          <div>
            ...
            <p className="text-gray-600">{teacher.Linkedin}</p>
          </div>
        </div>
      </div>
    </div>
